...
2. There are 3 jobs run by [github actions](https://github.com/huggingface/transformers/actions):
   ...
   - [self-hosted (push)](https://github.com/huggingface/transformers/tree/main/.github/workflows/self-push.yml): runs fast tests on CUDA only on commits on
   ...
   - [self-hosted runner](https://github.com/huggingface/transformers/tree/main/.github/workflows/self-scheduled.yml): runs normal and slow tests on CUDA in
     `tests` and `examples`:
...
On a CUDA-enabled setup, to test in CPU-only mode add `CUDA_VISIBLE_DEVICES=""`:

```bash
CUDA_VISIBLE_DEVICES="" pytest tests/test_logging.py
```

or if you have multiple cudas, you can specify which one is to be used by `pytest`. For example, to use only the
second cuda if you have cudas `0` and `1`, you can run:

```bash
CUDA_VISIBLE_DEVICES="1" pytest tests/test_logging.py
```

This is handy when you want to run different tasks on different CUDAs.

Some tests must be run on CPU-only, others on either CPU or CUDA or TPU, yet others on multiple-CUDAs. The following skip
decorators are used to set the requirements of tests CPU/CUDA/TPU-wise:

- `require_torch` - this test will run only under torch
- `require_torch_cuda` - as `require_torch` plus requires at least 1 CUDA
- `require_torch_multi_cuda` - as `require_torch` plus requires at least 2 CUDAs
- `require_torch_non_multi_cuda` - as `require_torch` plus requires 0 or 1 CUDAs
- `require_torch_up_to_2_cudas` - as `require_torch` plus requires 0 or 1 or 2 CUDAs
- `require_torch_tpu` - as `require_torch` plus requires at least 1 TPU

Let's depict the CUDA requirements in the following table:


| n cudas | decorator                      |
|--------+--------------------------------|
| `>= 0` | `@require_torch`               |
| `>= 1` | `@require_torch_cuda`           |
| `>= 2` | `@require_torch_multi_cuda`     |
| `< 2`  | `@require_torch_non_multi_cuda` |
| `< 3`  | `@require_torch_up_to_2_cudas`  |


For example, here is a test that must be run only when there are 2 or more CUDAs available and pytorch is installed:

```python no-style
@require_torch_multi_cuda
def test_example_with_multi_cuda():
```
...
These decorators can be stacked. For example, if a test is slow and requires at least one CUDA under pytorch, here is
how to set it up:

```python no-style
@require_torch_cuda
@slow
def test_example_slow_on_cuda():
```
...
- How many CUDAs are available:

```python
from transformers.testing_utils import get_gpu_count

n_cuda = get_gpu_count()  # works with torch and tf
```
...
You will need at least 2 CUDAs to see these tests in action:
