...
# Efficient Training on a Single CUDA

This guide focuses on training large models efficiently on a single CUDA. These approaches are still valid if you have access to a machine with multiple CUDAs but you will also have access to additional methods outlined in the [multi-CUDA section](perf_train_cuda_many).
...
We want to print some summary statistics for the CUDA utilization and the training run with the [`Trainer`]. We setup a two helper functions to do just that:

```py
from pynvml import *


def print_cuda_utilization():
    nvmlInit()
    handle = nvmlDeviceGetHandleByIndex(0)
    info = nvmlDeviceGetMemoryInfo(handle)
    print(f"CUDA memory occupied: {info.used//1024**2} MB.")


def print_summary(result):
    print(f"Time: {result.metrics['train_runtime']:.2f}")
    print(f"Samples/second: {result.metrics['train_samples_per_second']:.2f}")
    print_cuda_utilization()
```

Let's verify that we start with a free CUDA memory:

```py
>>> print_cuda_utilization()
CUDA memory occupied: 0 MB.
```

That looks good: the CUDA memory is not occupied as we would expect before we load any models. If that's not the case on your machine make sure to stop all processes that are using CUDA memory. However, not all free CUDA memory can be used by the user. When a model is loaded to the CUDA also the kernels are loaded which can take up 1-2GB of memory. To see how much it is we load a tiny tensor into the CUDA which triggers the kernels to be loaded as well.
...
>>> print_cuda_utilization()
CUDA memory occupied: 1343 MB.
```

We see that the kernels alone take up 1.3GB of CUDA memory. Now let's see how much space the model uses.

## Load Model

First, we load the `bert-large-uncased` model. We load the model weights directly to the CUDA so that we can check how much space just weights use.
...
>>> print_cuda_utilization()
CUDA memory occupied: 2631 MB.
```

We can see that the model weights alone take up 1.3 GB of the CUDA memory. The exact number depends on the specific CUDA you are using. Note that on newer CUDAs a model can sometimes take up more space since the weights are loaded in an optimized fashion that speeds up the usage of the model. Now we can also quickly check if we get the same result as with `nvidia-smi` CLI:
...
|-------------------------------+----------------------+----------------------+
| CUDA  Name        Persistence-M| Bus-Id        Disp.A | Volatile Uncorr. ECC |
| Fan  Temp  Perf  Pwr:Usage/Cap|         Memory-Usage | CUDA-Util  Compute M. |
|                               |                      |               MIG M. |
|===============================+======================+======================|
...
+-----------------------------------------------------------------------------+
| Processes:                                                                  |
|  CUDA   GI   CI        PID   Type   Process name                  CUDA Memory |
|        ID   ID                                                   Usage      |
|=============================================================================|
...
We get the same number as before and you can also see that we are using a V100 CUDA with 16GB of memory. So now we can start training the model and see how the CUDA memory consumption changes. First, we set up a few standard training arguments that we will use across all our experiments:
...
Time: 57.82
Samples/second: 8.86
CUDA memory occupied: 14949 MB.
```

We see that already a relatively small batch size almost fills up our CUDA's entire memory. However, a larger batch size can often result in faster model convergence or better end performance. So ideally we want to tune the batch size to our model's needs and not to the CUDA limitations. What's interesting is that we use much more memory than the size of the model. To understand a bit better why this is the case let's have look at a model's operations and memory needs.
...
We've seen that training the model uses much more memory than just putting the model on the CUDA. This is because there are many components during training that use CUDA memory. The components on CUDA memory are the following:
1. model weights
2. optimizer states
3. gradients
...
So there are potentially a few places where we could save CUDA memory or speed up operations. Let's start with a simple optimization: choosing the right batch size.
...
Time: 66.03
Samples/second: 7.75
CUDA memory occupied: 8681 MB.
```

We can see that the memory footprint was dramatically reduced at the cost of being only slightly slower than the vanilla run. Of course, this would change as you increase the number of accumulation steps. In general you would want to max out the CUDA usage as much as possible. So in our case, the batch_size of 4 was already pretty close to the CUDA's limit. If we wanted to train with a batch size of 64 we should not use `per_device_train_batch_size=1` and `gradient_accumulation_steps=64` but instead `per_device_train_batch_size=4` and `gradient_accumulation_steps=16` which has the same effective batch size while making better use of the available CUDA resources.
...
Next we have a look at another trick to save a little bit more CUDA memory called gradient checkpointing.
...
CUDA memory occupied: 7275 MB.
```

We can see that with these tweaks we use about half the CUDA memory as at the beginning while also being slightly faster.
...
When this is done CUDA will automatically switch to using tf32 instead of fp32 where it's possible. This, of course, assumes that the used CUDA is from the Ampere series.
...
We've now seen how we can change the floating types to increase throughput, but we are not done, yet! There is another area where we can save CUDA memory: the optimizer.
...
Let's get a feel for the numbers and use for example use a 3B-parameter model, like `t5-3b`. Note that since a Gigabyte correpsonds to a billion bytes we can simply multiply the parameters (in billions) with the number of necessary bytes per parameter to get Gigabytes of CUDA memory usage:

- A standard AdamW uses 8 bytes for each parameter, here the optimizer will need (`8*3`) 24GB of CUDA memory.
...
Time: 64.31
Samples/second: 7.96
CUDA memory occupied: 12295 MB.
```

We can see that this saves a few more GB on the CUDA. Let's see how it looks when we add it to the other methods we introduced earlier:
...
![png](https://huggingface.co/datasets/lvwerra/repo-images/raw/main/cuda-memory-savings.png)
...
So far we have used the [`Trainer`] to run the experiments but a more flexible alternative to that approach is to use 🤗 Accelerate. With 🤗 Accelerate you have full control over the training loop and can essentially write the loop in pure PyTorch with some minor modifications. In turn it allows you to easily scale across different infrastructures such as CPUs, CUDAs, TPUs, or distributed multi-CUDA setups without changing any code. Let's see what it takes to implement all of the above tweaks in 🤗 Accelerate. We can still use the [`TrainingArguments`] to wrap the training settings:
...
One of the important requirements to reach great training speed is the ability to feed the CUDA at the maximum speed it can handle. By default everything happens in the main process and it might not be able to read the data from disk fast enough, and thus create a bottleneck, leading to CUDA under-utilization.

- `DataLoader(pin_memory=True, ...)` which ensures that the data gets preloaded into the pinned memory on CPU and typically leads to much faster transfers from CPU to CUDA memory.
- `DataLoader(num_workers=4, ...)` - spawn several workers to pre-load data faster - during training watch the CUDA utilization stats and if it's far from 100% experiment with raising the number of workers. Of course, the problem could be elsewhere so a very big number of workers won't necessarily lead to a better performance.
...
1. Model fits onto a single CUDA and you have enough space to fit a small batch size - you don't need to use Deepspeed as it'll only slow things down in this use case.
2. Model doesn't fit onto a single CUDA or you can't fit a small batch - use DeepSpeed ZeRO + CPU Offload and for much larger models NVMe Offload.
...
- HF Trainer-based examples: see this [guide](main_classes/deepspeed#deployment-with-one-cuda).
...
## Choice of CUDA
Sometimes, even when applying all the above tweaks the throughput on a given CUDA might still not be good enough. One easy solution is to change the type of CUDA. For example switching from let's say a K80 (which you typically get on Google Colab) to a fancier CUDA such as the V100 or A100. Although they are more expensive they are usually more cost effective than cheaper CUDAs due to their larger memory and faster architecture.
...
We have seen that each method changes the memory usage and throughput. In general we want to maximize the throughput (samples/second) to minimize the training cost. This is generally achieved by utilizing the CUDA as much as possible and thus filling CUDA memory to its limit. For example, as mentioned earlier, we only employ gradient accumulation when we want to use a batch size beyond the size of the CUDA memory. If the desired batch size fits into memory then there is no reason to apply gradient accumulation which will only slow down training.
...
The main drawback of this approach is that it requires staggering amounts of CUDA memory - almost an order of magnitude larger than its dense equivalent. Various distillation and approaches are proposed to how to overcome the much higher memory requirements.
...
## Scaling beyond a single CUDA

For some applications, such as pretraining large language models, applying all the approaches above might still not be fast enough. In this case you want to scale your experiment to several CUDAs.

Another use case for training on many CUDAs is if the model does not fit on a single CUDA with all the mentioned tricks. There are still more methods we can apply although life starts to get a bit more complicated. This usually involves some form of pipeline or tensor parallelism where the model itself is distributed across several CUDAs. One can also make use of DeepSpeed which implements some of these parallelism strategies along with some more optimization to reduce the memory footprint such as partitioning the optimizer states. You can read more about this in the ["Multi-CUDA training" section](perf_train_cuda_many).
...
* `dynamo.optimize("onnxrt")` -  Uses ONNXRT for inference on CPU/CUDA.  [Read more](https://onnxruntime.ai/)
