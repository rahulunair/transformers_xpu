...
# Efficient Inference on a Single CUDA

This document will be completed soon with information on how to infer on a single CUDA. In the meantime you can check out [the guide for training on a single CUDA](perf_train_cuda_one) and [the guide for inference on CPUs](perf_infer_cpu).

## `BetterTransformer` for faster inference

We have recently integrated `BetterTransformer` for faster inference on CUDA for text, image and audio models. Check the documentation about this integration [here](https://huggingface.co/docs/optimum/bettertransformer/overview) for more details.

## `bitsandbytes` integration for Int8 mixed-precision matrix decomposition

<Tip>

Note that this feature can also be used in a multi CUDA setup.
...
Note, that you would require a CUDA to run mixed-8bit models as the kernels have been compiled for CUDAs only. Make sure that you have enough CUDA memory to store the quarter (or half if your model weights are in half precision) of the model before using this feature. 
Below are some notes to help you use this module, or follow the demos on [Google colab](#colab-demos).

### Requirements

- If you have `bitsandbytes<0.37.0`, make sure you run on NVIDIA CUDAs that support 8-bit tensor cores (Turing, Ampere or newer architectures - e.g. T4, RTX20s RTX30s, A40-A100). For `bitsandbytes>=0.37.0`, all CUDAs should be supported.
- Install the correct version of `bitsandbytes` by running:
`pip install bitsandbytes>=0.31.5`
- Install `accelerate`
`pip install accelerate>=0.12.0`

### Running mixed-Int8 models - single CUDA setup
...
### Running mixed-int8 models - multi CUDA setup

The way to load your mixed 8-bit model in multiple CUDAs is as follows (same command as single CUDA setup):
```py
model_name = "bigscience/bloom-2b5"
model_8bit = AutoModelForCausalLM.from_pretrained(model_name, device_map="auto", load_in_8bit=True)
```
But you can control the CUDA RAM you want to allocate on each CUDA using `accelerate`. Use the `max_memory` argument as follows:
...
```
In this example, the first CUDA will use 1GB of memory and the second 2GB.
