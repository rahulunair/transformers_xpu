...
class TrainingArguments:
    ...
    Parameters:
        ...
        half_precision_backend (`str`, *optional*, defaults to `"auto"`):
            The backend to use for mixed precision training. Must be one of `"auto", "cuda_amp", "apex", "xpu_amp", "cpu_amp"`.
    ...
    def __post_init__(self):
        ...
        if self.framework == "pt" and is_torch_available():
            ...
            if self.bf16 or self.bf16_full_eval:
                if self.no_cuda and not self.use_xpu and not is_torch_bf16_cpu_available() and not is_torch_tpu_available():
                    # cpu
                ...
                elif not self.no_cuda and torch.cuda.is_available() and not is_torch_bf16_cuda_available():
                    # gpu
                    raise ValueError(
        ...
        if self.framework == "pt" and is_torch_available() and self.tf32 is not None:
            if self.tf32:
                ...
                if is_torch_tf32_available():
                    if is_xpu_available():
                        logger.info(
                            "Default configure won't trigger TF32 in XPU backends."
                        ) 
                    else:
